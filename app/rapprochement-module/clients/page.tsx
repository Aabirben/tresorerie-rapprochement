...
export default function ClientsPage() {
  ...
  const filteredClients = clients.filter(
    (c) =>
      (statusFilter === 'Tous' || c.statut === statusFilter) &&
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center gap-4">
        ...
        <Select value={statusFilter} onValueChange={(value) => setStatusFilter(value as 'Tous' | 'Actif' | 'Inactif')}>
            ...
            <TableBody>
              {filteredClients.length === 0 ? (
                ...
                paginatedClients.map((client) => (
                  <TableRow
                    key={client.id}
                    className={`border-[#DDE3EF] ${client.statut === 'Inactif' ? 'opacity-50 bg-gray-50' : 'hover:bg-[#F4F6FB]'}`}
                  >
                    <TableCell className={`font-medium ${client.statut === 'Inactif' ? 'text-gray-400' : 'text-[#1B2E5E]'}`}>
                      {client.raisonSociale}
                    </TableCell>
                    <TableCell className={`font-mono text-sm ${client.statut === 'Inactif' ? 'text-gray-400' : 'text-[#64748B]'}`}>
                      {client.ice}
                    </TableCell>
                    <TableCell className={client.statut === 'Inactif' ? 'text-gray-400' : 'text-[#64748B]'}>{client.segment || '—'}</TableCell>
                    <TableCell className={client.statut === 'Inactif' ? 'text-gray-400' : 'text-[#64748B]'}>{client.ville}</TableCell>
                    <TableCell className={client.statut === 'Inactif' ? 'text-gray-400' : 'text-[#64748B]'}>{client.modePaiement}</TableCell>
                    <TableCell className={client.statut === 'Inactif' ? 'text-gray-400' : 'text-[#64748B]'}>{client.delaiPaiement}j</TableCell>
                    <TableCell>
